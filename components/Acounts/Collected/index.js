...
export default function Collected ({ marketContract, tokenContract, accounts }) {
   ...
   useEffect(() => {
      (async () => {
         setLoading(true)
         try {
            const items = await marketContract.methods.fetchMyNFTs().call({
               from: accounts[0]
            });
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
         ...
         }
         {
            ...
            myNFTs.map((data, index) => {
               ...
               const address = sortAddress(data.owner);
               return (
                  <Card
                     key={index}
                     name={data.name}
